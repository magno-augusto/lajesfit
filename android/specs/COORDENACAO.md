...
>
> **Atualização 2026-07-15: Codex descontinuado.** A assinatura do Codex terminou; a partir
> desta data todo o trabalho (planejar, implementar, validar, commitar) é do **Claude Code**.
> O quadro e o diário continuam como registro de estado entre sessões. Pedido do usuário na
...
|---|--------|------|------|---------------------|--------|-------|
| 9 | Incentivo ao Health Connect no Feed + desconectar Strava automaticamente | pedido direto do usuario | Claude | `app/src/main/kotlin/com/lajesfit/android/feature/feed/FeedScreen.kt`, `app/src/main/kotlin/com/lajesfit/android/feature/workouts/WorkoutRepository.kt`, `app/src/main/kotlin/com/lajesfit/android/feature/workouts/WorkoutsViewModel.kt`, `app/src/main/kotlin/com/lajesfit/android/navigation/LajesFitNavGraph.kt`, `../src/features/workouts/strava.server.ts`, `../src/features/workouts/strava-api.ts`, `../src/routes/api/strava/disconnect.ts` (novo) | **concluída** (`:app:compileDebugKotlin`/`installDebug` OK, `npx tsc --noEmit` OK no web; banner testado no device — confirmado que NAO aparece no Galaxy J7 Prime, Health Connect UNAVAILABLE nesse Android 8.1, como esperado) | Banner no Feed (novo `FeedViewModel.healthConnectStatus`) some quando o status vira READY; ao conectar o Health Connect (`WorkoutsViewModel.onHealthPermissionsResult`), o Android chama `POST /api/strava/disconnect` (novo endpoint web, autenticado por Bearer token — mesma logica de `disconnectStrava` do botao de Configuracoes, extraida para `disconnectStravaForUser` em `strava.server.ts`) para liberar a vaga de conexao limitada da API do Strava. **Pendente**: deploy do web pra produção (o endpoint novo só existe local ainda) e verificação visual do banner num aparelho com Health Connect disponível (ex.: Galaxy Tab S6 Lite) — o J7 Prime conectado nesta sessão não suporta Health Connect. |
| 7 | Polimento visual global do app Android | pedido direto do usuario | Claude (assumiu de Codex em 2026-07-09) | `app/src/main/kotlin/com/lajesfit/android/ui/theme/*`, `app/src/main/res/font/*`, `app/src/main/kotlin/com/lajesfit/android/MainActivity.kt`, `app/src/main/kotlin/com/lajesfit/android/ui/components/*`, `app/src/main/kotlin/com/lajesfit/android/feature/auth/*`, `app/src/main/kotlin/com/lajesfit/android/feature/feed/*`, `app/src/main/kotlin/com/lajesfit/android/feature/diet/*`, `app/src/main/kotlin/com/lajesfit/android/feature/workouts/*`, `app/src/main/kotlin/com/lajesfit/android/feature/challenges/*`, `app/src/main/kotlin/com/lajesfit/android/feature/profile/*`, `app/src/main/kotlin/com/lajesfit/android/feature/settings/*`, `specs/COORDENACAO.md` | **concluída** (verificada em device real 2026-07-15: Login, Feed, Dieta, Treinos, Perfil, Busca, Configuracoes conferidos por screenshot no Galaxy J7 Prime via adb - todas batem com o design pretendido) | Iteração 2026-07-10 (Claude): redesign do tema para **fidelidade ao design system do web** (`../src/styles.css`). Paleta reescrita a partir dos tokens oklch — fundo quase-branco + cards brancos, secundaria marrom-ambar e terciaria clay no lugar do verde/azul (verde só em success); cantos mais arredondados (medium 8→16dp); fonte Bebas Neue (OFL) em `res/font` aplicada aos estilos display (marca na top bar + telas de auth). Contexto anterior: commits `e495726`/`cf8823d`. **Iteracao 2026-07-15 (Claude):** usuario reportou que o app seguia muito diferente do web nas 4 areas (Dieta, Treinos, Feed, Perfil/Login/Config) — gap de composicao de tela, nao de tema. Reescritas todas as **8 sub-partes**, uma tela (ou familia pequena) por commit, cada uma com `:app:assembleDebug` BUILD SUCCESSFUL (maquina "casa" com Android Studio/JBR): (1) Dieta `3012c00`, (2) Treinos `145cda1`, (3) Feed `cc70ad4`, (4) Perfil `8dd0149`, (5) Busca de pessoas `8e03f75`, (6) Login+Cadastro `18c3275`, (7) Recuperacao de conta (esqueci senha/cadastrar e-mail/nova senha) `d3cfc5e`, (8) Configuracoes `6d466ae`. Padrao aplicado em todas: cards brancos (`surfaceContainerLow` + `BorderStroke(1.dp, outline)` + `elevation 1.dp`), numeros/titulos de destaque em `fontFamily = BebasNeue`, badges/icones circulares `primary.copy(alpha=0.1f)` (e `LajesFitSuccess` para o badge "Dieta" do Feed), avatar em anel `primary/30` (Perfil/Config). Decisoes de escopo mantidas: Strava (link/logo/OAuth), `WeeklyWorkoutChart`, compartilhar post, upload de midia no CreatePost e push web ficam de fora (Health Connect e FCM/M8 substituem). **Pendente: verificacao visual em device real** (as 8 sub-partes so foram validadas por `assembleDebug` + leitura/@Preview, nao por screenshot no aparelho) — proximo passo antes de fechar a tarefa como concluida. |
...
|------|--------|------|--------|-------|
| 2026-07-11 | Debug local convive com release da Play (`applicationIdSuffix`) | Claude + usuario | concluída | Adicionado buildType `debug` com `applicationIdSuffix = ".debug"` + `versionNameSuffix = "-debug"` em `app/build.gradle.kts`, para o build local (debug key) instalar como `com.lajesfit.app.debug` **lado a lado** com o release `com.lajesfit.app` do teste interno, sem conflito de assinatura — dev iterando novas funcoes sem tocar no app da Play. Seguro: sem FCM/google-services (nenhum match de package exigido) e OAuth usa scheme fixo `lajesfit://auth` (independe do applicationId). `:app:installDebug` OK (JBR + cache repo-local), instalado e aberto no device via **ADB WiFi** (`192.168.0.134:5555`, J7 Prime), `ResumedActivity` = debug, sem crash no logcat; ambos os processos vivos. Caveat pendente: Google Sign-In no debug pode exigir registrar o SHA-1 da debug key no Google Cloud/Supabase (independe do sufixo). Sem commit ainda (mudanca de config aguardando decisao do usuario). |
