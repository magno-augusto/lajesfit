import { Activity, Heart, Link2, MessageCircle, MoreHorizontal, Share2, Trash2 } from "lucide-react";
...
export function PostCard({
  ...
  const shareText = (() => {
    ...
    return post.content ? `${authorName} no Lajes Fit: ${post.content}` : `${authorName} no Lajes Fit`;
  })();
  const shareUrl = "https://lajesfit.vercel.app";

  async function openShareSheet() {
    setShareOpen(true);
    if (sharePreparedRef.current) return;
    ...
    if (mediaUrl) {
      try {
        ...
        setShareFile(new File([blob], `lajesfit-post.${extension}`, { type: blob.type }));
      } catch {
        setShareFile(null);
      }
    }
  ...
  async function shareViaInstagram() {
    if (navigator.clipboard) {
      ...
      toast.success("Link copiado! Abra o Instagram e cole nos Stories ou Direct.");
    }
    setShareOpen(false);
  }
  ...
  async function shareNative() {
    const canShareFiles =
      shareFile && typeof navigator.canShare === "function" && navigator.canShare({ files: [shareFile] });
    try {
      if (navigator.share) {
  ...
  return (
    <article className="bg-card rounded-2xl border shadow-card overflow-hidden">
      ...
      <footer className="flex items-center gap-1 p-2 border-t mt-2">
        ...
        {canShare && (
          <Button variant="ghost" size="sm" onClick={() => void openShareSheet()}>
