...
import { deletePost, fetchProfilePosts, type FeedPost } from "@/lib/feed";
...
function ProfilePage() {
  ...
  const load = useCallback(async () => {
    ...
    if (isMe) await loadIncomingRequests(user.id);
  ...
  async function loadIncomingRequests(userId: string) {
    ...
    setIncomingRequests(
      (requests ?? [])
        ...
        .filter((request): request is IncomingRequest => Boolean(request)),
    );
  }
  ...
  async function followOrRequest() {
    ...
    try {
      if (profile.is_private) {
        await supabase
          .from("follow_requests")
          .insert({ requester_id: user.id, requested_id: profile.id });
